...
@app.get("/phonebook/find",response_model=PhoneBookOut)
...
@app.get("/phonebook/{pb_id}",response_model=PhoneBookOut)
def get_pb_id(pb_id: int):
    conn = sqlite3.connect("phonebook.db")
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM phonebook WHERE id = ?",(pb_id,))
    row = cursor.fetchone()
    conn.close()
    if row:
        return {"id": row[0], "name": row[1], "phone": row[2], "address": row[3]}
...
@app.put("/phonebook/{pb_id}",response_model=PhoneBook)
def update_pb(pb_id: int, pb: PhoneBook):
    conn = sqlite3.connect("phonebook.db")
    cursor = conn.cursor()
    cursor.execute("UPDATE phonebook set name = ?, phone = ?, address = ? WHERE id = ?",(pb.name,pb.phone,pb.address,pb_id))
    conn.commit()
    if cursor.rowcount == 0:
        conn.close()
        raise HTTPException(status_code=404,detail="PhoneBook not found")
    conn.close()
    return {**pb.dict(),"id":pb_id}